<script setup>
let urlCartDetail = 'http://localhost/vuejs-server/api.php/cart-detail';
let urlCheckOut = 'http://localhost/vuejs-server/api.php/check-out';
let urlOrder = 'http://localhost/vuejs-server/api.php/show_order';
let urlChangeStatus = 'http://localhost/vuejs-server/api.php/change-status';

import axios from 'axios';
import { onMounted, ref } from 'vue';

const order = ref({});

const convertPrice = (number) => {
    return number.toLocaleString("vi-VN");
}

const cartDetail = ref([]);
const callAPI = async () => {
    try {
        let user_id = JSON.parse(localStorage.getItem('userLogin')).id;
        let response = await axios.get(urlOrder + "?user_id=" + user_id);

        if (response.status == 200) {
            order.value = response.data;
        }
    } catch (error) {
        console.log('Lỗi API');

    }
}

onMounted(() => {
    callAPI();
})

const convertDate = (input) => {
    const datePart = input.split("-");
    if (datePart.length !== 3) return "validate";
    const year = datePart[0];
    const month = datePart[1];
    const day = datePart[2];

    return `${day}/${month}/${year}`
}

const handleCancel = async (order_id) => {
    const check = confirm('Bạn có chắc muốn xóa hay không?');
    if (check) {
        try {
            let response = await axios.get(urlChangeStatus + '?order_id=' + order_id + '&status_code=3');
            if(response.status == 200){
                alert('Hủy đơn hàng thành công');
                callAPI();
            }
        } catch (error) {
            console.log(error);

        }
    }

}
</script>

<template>
    <div class="container">
        <div class="row mt-5">
            <div class="col-10">
                <h4>Danh sách sản phẩm</h4>
                <table class="table">
                    <thead>
                        <tr>
                            <th>STT</th>
                            <th>Ngày đặt hàng</th>
                            <th>Trang Thái</th>
                            <th>Tổng tiền</th>
                            <th>Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="(item, index) in order" :key="index">
                            <td>{{ index + 1 }}</td>
                            <td>{{ convertDate(item.order_date) }}</td>
                            <td>
                                <span v-if="item.status == 1" class="badge text-bg-warning">Chưa xác nhận</span>
                                <span v-else-if="item.status == 2" class="badge text-bg-info">Đã xác nhận</span>
                                <span v-else-if="item.status == 3" class="badge text-bg-danger">Đã hủy</span>
                                <span v-else-if="item.status == 4" class="badge text-bg-success">Đang Giao</span>
                                <span v-else class="badge text-bg-success">đã giao</span>
                            </td>
                            <td>{{ convertPrice(item.total_price) }} VNĐ</td>
                            <td>
                                <RouterLink :to="`order-detail/${item.order_id}`" class="btn btn-info">Xem chi tiết</RouterLink>
                                <button v-if="item.status == 1" class="btn btn-danger"
                                    @click="handleCancel(item.order_id)">Hủy đơn</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</template>